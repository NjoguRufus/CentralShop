 ...
 */
function formatReceiptEscPos(order: OrderData): string {
  const ESC = '\x1B';
  const GS = '\x1D';
 ...
 */
function formatReceiptHTML(order: OrderData): string {
  return `
<!DOCTYPE html>
<html>
<head>
 ...
 */
export async function printReceipt(orderData: OrderData, window: BrowserWindow | null): Promise<void> {
  try {
